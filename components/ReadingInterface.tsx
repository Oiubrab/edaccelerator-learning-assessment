...
export default function ReadingInterface({ questions }: ReadingInterfaceProps) {
  ...
  }, [showFeedback, isComplete, canContinue, handleNext]);
  ...
  if (isComplete) {
    ...
    if (showHistory) {
      const history = JSON.parse(localStorage.getItem('quiz-history') || '[]') as AttemptHistory[];
      const sortedHistory = [...history].reverse(); // Most recent first
      
      const bestScore = history.length > 0 ? Math.max(...history.map((h: AttemptHistory) => h.percentage)) : 0;
      const avgScore = history.length > 0 ? Math.round(history.reduce((sum: number, h: AttemptHistory) => sum + h.percentage, 0) / history.length) : 0;
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="space-y-3 mb-8">
              {sortedHistory.length > 0 ? (
                sortedHistory.map((attempt: AttemptHistory, attemptIndex: number) => {
